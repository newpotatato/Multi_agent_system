...
from config.config import get_spsa_config, get_lvp_config
import random
import numpy as np
import math
...
class Broker:
    ...
    def calculate_ui(self, neighbors, all_brokers):
        ...
        lvp_config = get_lvp_config()
        h = lvp_config.h
        gamma = lvp_config.gamma
    ...
    def update_parameters(self):
        ...
        # Параметры SPSA
        spsa_config = get_spsa_config()
        alpha = spsa_config.alpha  # Скорость обучения
        beta = spsa_config.beta    # Размер возмущения
